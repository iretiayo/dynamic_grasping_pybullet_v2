...
class RobotController(object):
    def __init__(self,
        ...
        self.EEF_LINK_INDEX = pu.link_from_name(self.id, self.robot_config['eef_link'])

        # for motion planning
        self.arm_difference_fn = pu.get_difference_fn(
            self.id, self.robot_config['group_index']['arm'])
        ...
        self.link_pairs = pu.get_self_link_pairs(
            self.id,
            self.robot_config['group_index']['arm'])
    ...
    def reset(self):
        ...
        self.set_gripper_joints(self.robot_config['open_position'])
    ...
    def set_arm_joints(self, joint_values):
        pu.set_joint_positions(self.id, self.robot_config['group_index']['arm'], joint_values)
        pu.control_joints(self.id, self.robot_config['group_index']['arm'], joint_values)

    def control_arm_joints(self, joint_values):
        pu.control_joints(
            body=self.id,
            joints=self.robot_config['group_index']['arm'],
            positions=joint_values)

    def set_gripper_joints(self, joint_values):
        pu.set_joint_positions(
            self.id, self.robot_config['group_index']['gripper'], joint_values)
        pu.control_joints(self.id, self.robot_config['group_index']['gripper'], joint_values)
    ...
    def close_gripper(self, realtime=False, duration=2):
        waypoints = self.plan_gripper_joint_values(
            self.robot_config['closed_position'], duration=duration)
        self.execute_gripper_plan(waypoints, realtime)

    def open_gripper(self, realtime=False, duration=2):
        waypoints = self.plan_gripper_joint_values(
            self.robot_config['open_position'], duration=duration)
        self.execute_gripper_plan(waypoints, realtime)
    ...
    def get_arm_ik_pybullet(self, pose_2d, report_time=False, max_iters=10000, residual=.0001):
    ...
    def get_arm_joint_values(self):
        return pu.get_joint_positions(self.id, self.robot_config['group_index']['arm'])

    def get_gripper_joint_values(self):
        return pu.get_joint_positions(self.id, self.robot_config['group_index']['gripper'])
        ...
        for i, closest_points_to_other in enumerate(
            ...
            for point in closest_points_to_other:
                if point[8] < collision_distance:
                    ...
                    return True, (pu.get_body_name(point[1]), pu.get_body_name(point[2]), point[3], point[4])
        if self_collision:
            # Self Collision
