...
import ray
ray.init()
from mico_controller import IKSolver
...
class DynamicGraspingWorld:
    def __init__(self,
        ...
        self.ik_solvers = [IKSolver.remote() for _ in range(self.max_check)]
    ...
    def plan_grasp(self, target_pose, old_grasp_idx):
        ...
        pregrasp_iks_exists = [len(jv) > 0 for jv in pregrasp_iks_ray]
        if np.any(pregrasp_iks_exists):
            ...
            grasp_iks_exists = [len(jv) > 0 for jv in grasp_iks_ray]
